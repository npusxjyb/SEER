...
clipmodel, preprocess = clip.load('ViT-B/16', device)
...
class twitter_dataset(data.Dataset):
    def __init__(self, root_path='data/twitterdata/twitter', is_train=True):
        ...
        self.local_path = 'data/twitterdata/dataset'

        # Read CSV file to populate label_dict
        wb = pandas.read_csv(self.local_path+'/{}_twitter.csv'.format('train' if is_train else 'test'))

        # Populate label_dict with records from the CSV file
        for i in tqdm(range(len(wb))):
            images_name = str(wb.iloc[i,1])
            label = int(wb.iloc[i,2])
            content = str(wb.iloc[i,0])
            sum_content = str(wb.iloc[i,0])
            prompt = str(wb.iloc[i, 4])
            record = {}
    ...
    def __getitem__(self, index):
        ...
        images, label, content, sum_content, prompt= record['images'], record['label'], record['content'], record['sum_content'],record['prompt']
        if label == 0:
            LABLEF = 'images'
        else:
            LABLEF = 'images'
        ...
        return (content, self.swin(img_GT, return_tensors="pt").pixel_values, self.preprocess(img_pro), sum_content, prompt ), label
        #return (content, self.swin(img_GT, return_tensors="pt").pixel_values, self.preprocess(img_GT), sum_content), label
...
def collate_fn(data):
    ...
    prompt_data = token.batch_encode_plus(batch_text_or_text_pairs=prompt,
                                          truncation=True,
                                          padding='max_length',
                                          max_length=300,
                                          return_tensors='pt',
    ...
    prompt_token_type_ids = prompt_data['token_type_ids']

    image = torch.stack(image).squeeze()
    imageclip = torch.stack(imageclip)
    labels = torch.LongTensor(labels)
    return input_ids, attention_mask, token_type_ids, image, imageclip, textclip, labels, prompt_input_ids, prompt_attention_mask, prompt_token_type_ids
