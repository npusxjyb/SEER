...
class weibo_dataset(data.Dataset):
    def __init__(self, root_path='data/weibodata/weibo', is_train=True):
        ...
        self.local_path = 'data/weibodata/dataset'
        ...
        for i in tqdm(range(len(wb))):
            images_name = str(wb.iloc[i,0])
            label = int(wb.iloc[i,1])
            content = str(wb.iloc[i,2])
            sum_content = str(wb.iloc[i,3])
